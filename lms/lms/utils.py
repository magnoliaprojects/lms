...
# charge = paystackapi.charge
# response = charge.start_charge(
#             email="ijenrandy2@gmail.com",
#             amount=50000,)
# print("The Charge:---"*6)
# print(response)
...
def get_payment_options(doctype, docname, phone, country):
    ...
    order = create_order(details.amount, details.currency,metadata)
    print("Order created:-------",order)

    options = {
        "key_id": frappe.db.get_single_value("LMS Settings", "paystack_secret"),
        "name": frappe.db.get_single_value("Website Settings", "app_name"),
        "description": _("Payment for {0} course").format(details["title"]),
        "amount": cint(details.amount) * 100,
...
def get_client():
    ...
    print("MY secret:--------",paystack_secret_key)
    paystack_obj = paystack.Paystack(secret_key=str(paystack_secret_key).strip())

    if not paystack_obj and not paystack_secret_key:
        frappe.throw(
            _(
...
def create_order(amount, currency,metadata):
    # charge_obj = client.charge
    order_data = {'data':{'amount':amount,'currency':currency,'metadata':metadata}}
    try:
        return order_data
        # return charge_obj.start_charge(
        #     email = "ijenrandy2@gmail.com",
        #     amount = amount * 100,
        #     metadata={
        #         "currency":currency,
        ...
        frappe.throw(
            ...
            ).format(e, amount, metadata['currency'], amount)
        )
